...
ticker.setProgress(0.5);   // progress-driven alias of seekFraction
ticker.progress;           // current 0..1
ticker.loop = false;
```
...
Map any external 0..1 signal onto the timeline without caring about duration units:
...
const t = progressToTime(doc.duration, 0.35);
const tree = sample(doc, t);

// browser helpers
scrollProgress(document.documentElement); // element scroll range → 0..1
viewportProgress(sectionEl);              // element vs viewport → 0..1
```

| Export | Role |
| --- | --- |
| `clampProgress` | Clamp to 0..1 |
| `progressToTime` | `progress × duration` |
| `timeToProgress` | `time / duration` |
| `scrollProgress` | Element scrollTop range → 0..1 |
| `viewportProgress` | Element travel through the viewport → 0..1 |
...
| Progress | `clampProgress`, `progressToTime`, `timeToProgress`, `scrollProgress`, `viewportProgress` |
